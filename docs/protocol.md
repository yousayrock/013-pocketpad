...
```json
{
  "v": 1,
  "sensitivity": 1.4,
  "pages":         [{"id": "trackpad|macro|youtube|claude", "enabled": true}, ...],
  "bottomButtons": [{"id": "enter|backspace|keyboard|alttab|win", "enabled": true}, ...],
  "deck": [{"label": "コピー", "icon": "copy", "color": 4278251519,
            "message": {"type": "shortcut", "keys": ["ctrl","c"]}}, ...],
  "claudeDeck": [{"label": "Enter", "icon": "keyboard_return", "color": 4278251519,
                  "message": {"type": "shortcut", "keys": ["enter"]}}, ...]
}
```

`claudeDeck`はClaude Codeページ（`claude`）用のコマンドボタン。スキーマは`deck`と同一（`DeckButton`の配列）で、macro型の`message`で「テキスト入力→Enter」のような複数ステップも1ボタンにまとめられる。
