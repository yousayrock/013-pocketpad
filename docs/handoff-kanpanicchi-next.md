...
### 4. TODOパネルのフェーズ折り畳み（#23、未着手）
タスクが増えてくると長くなるため、フェーズ単位で折り畳めるようにしたい。
TaskCreate/TaskUpdateのtool_inputにはphase情報が無いため、フェーズをどう識別
させるか（subjectへの軽いプレフィックス表記など）から検討が必要。
